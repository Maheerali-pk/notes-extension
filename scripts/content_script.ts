...
const qsa = (str: string) => document.querySelectorAll(str) as NodeListOf<HTMLElement>;
...
interface CEProps {
   ...
   props: {
      [k in keyof (HTMLElement & HTMLInputElement & HTMLTextAreaElement)]?: (HTMLElement &
         HTMLInputElement &
...
const allElements: ElmNameObj<"notesHeader"> = {};
...
//    name: "link",
//    children: [
//       { tag: "b", props: {}, name: "bold" },
//       { tag: "u", props: {}, name: "underline" },
//    ],
// } as const;
type ElmNameObj<names extends string> = { [k in names]?: HTMLElement };
const ce = ({ props, tag, children, name }: CEProps, elementsObj?: { [k in string]: HTMLElement }) => {
   const elm = document.createElement(tag);

   Object.entries(props).forEach(([k, v]) => {
      if (k === "style") {
         Object.entries(v as CSSStyleDeclaration).forEach(([k, v]) => {
            (elm.style as any)[k] = v;
         });
      } else {
         (elm as any)[k] = v;
      }
...
type TabName = "all" | "unread";

const filterChatList = () => {
   const activeTab = qs(".ext-tab.active") as HTMLButtonElement;
   if (activeTab) {
      const tabName = activeTab.name as TabName;
      if (tabName === "all") {
         store.Chat.models.forEach((x: any) => {
...
const createNotesWrapper = () => {
   const onSaveNote = () => {
      const notesInput = qs(".ext-add-input") as HTMLTextAreaElement;
      const note = createNote({ date: new Date(), text: notesInput.value });
      const body = qs(".ext-notes-body");
      body.appendChild(note);
      notesInput.value = "";
      notesInput.focus();
      (qs(".ext-add-btn") as HTMLButtonElement).disabled = true;
   };
            ...
            children: [
               {
                  ...
                  props: {
                     oninput: (e) => {
                        ...
                        const btn = qs(".ext-add-btn") as HTMLButtonElement;
...
const createNote = ({ date, text }: INote) => {
   const save = () => {
      const noteText = wrapper.querySelector(".note-text") as HTMLElement;
                  ...
                  children: [
                     {
                        ...
                        props: {
                           ...
                           onclick: (e) => {
                              ...
                              if (wantToDeleteNote) wrapper.remove();
                           },
                           innerHTML: "X",
                        },
                     },
